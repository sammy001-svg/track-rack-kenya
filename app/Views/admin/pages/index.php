<?php /** @var array $pages */ ?>

<section class="a-panel">
  <div class="a-panel__head">
    <h2>Website pages</h2>
    <p>The written content behind Heritage, Customer Care and the legal pages.</p>
  </div>

  <div class="a-table-wrap">
    <table class="a-table">
      <thead>
        <tr>
          <th>Page</th>
          <th>URL</th>
          <th>Status</th>
          <th>Last updated</th>
          <th class="a-table__actions">Actions</th>
        </tr>
      </thead>
      <tbody>
        <?php foreach ($pages as $page): ?>
          <?php $publicUrl = $page['slug'] === 'heritage' ? '/heritage' : '/page/' . $page['slug']; ?>
          <tr>
            <td>
              <a class="a-strong" href="<?= e(url('/admin/pages/' . $page['id'] . '/edit')) ?>"><?= e($page['title']) ?></a>
              <?php if ($page['subtitle']): ?>
                <div class="a-cell-media__meta"><?= e(excerpt($page['subtitle'], 60)) ?></div>
              <?php endif; ?>
            </td>
            <td class="a-muted"><?= e($publicUrl) ?></td>
            <td>
              <span class="a-badge a-badge--<?= (int) $page['is_active'] === 1 ? 'live' : 'draft' ?>">
                <?= (int) $page['is_active'] === 1 ? 'Live' : 'Hidden' ?>
              </span>
            </td>
            <td class="a-faint a-nowrap"><?= e(pretty_date($page['updated_at'], true)) ?></td>
            <td class="a-table__actions">
              <div class="a-actions">
                <a class="a-icon-btn" href="<?= e(url($publicUrl)) ?>" target="_blank" rel="noopener" title="View on the site">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M14 4h6v6M20 4l-9 9M18 14v5a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V7a1 1 0 0 1 1-1h5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </a>
                <a class="a-icon-btn" href="<?= e(url('/admin/pages/' . $page['id'] . '/edit')) ?>" title="Edit">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M15 4l5 5-11 11H4v-5L15 4z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/></svg>
                </a>
              </div>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
  </div>
</section>
